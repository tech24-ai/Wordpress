...
function wpqa_insert_statement($response,$user_id,$_coupon = '',$_coupon_value = '') {
	...
	if ($post_id == 0 || is_wp_error($post_id)) {
		...
		if (isset($response['item_no']) && isset($response['custom'])) {
			...
			if ($response['item_no'] == 'pay') {
				$payment_kind = get_post_meta($str_replace,prefix_meta.'payment_kind',true);
				$payment_action = get_post_meta($str_replace,prefix_meta.'payment_action',true);
				if ($payment_action == "download") {
					$payment_file = get_post_meta($str_replace,prefix_meta.'payment_file',true);
					$variables["payment_file"] = $payment_file;
				}
			}
			if (($response['item_no'] == 'subscribe' || $response['item_no'] == 'subscribe-module' || ($response['item_no'] == 'pay' && isset($payment_kind) && $payment_kind == 'subscribe')) && $str_replace != 'lifetime') {
				$variables["payment_type"] = 'subscribe';
			}
...
function wpqa_statement_columns($old_columns) {
	...
	$columns["price_s"]     = esc_html__("Price","wpqa")." - (".esc_html__("coupon","wpqa").")";
...
function wpqa_statement_custom_columns($column) {
	...
	if ($payment_method == "Stripe" || $payment_method == "woocommerce") {
	...
	switch ( $column ) {
		...
		case 'transaction' :
			...
			echo ($payment_method == "woocommerce" && $payment_id != ""?"<a href='".admin_url('post.php?post='.$payment_id.'&action=edit')."' target='_blank'>".$payment_id." <span class='gray-span'>".esc_html__("WooCommerce Order","wpqa")."</span></a>":($item_transaction != ""?esc_html($item_transaction):"---").($payment_method != ""?" <span class='gray-span ".($payment_method == "PayPal"?"paypal-span":"payment_method")."'>".esc_html($payment_method)."</span>":"").($original_transaction != ""?" <span class='gray-span margin_l_20'>".esc_html($original_transaction)."</span>":""));
		break;
		case 'email' :
...
function wpqa_payment_subscribe_filter() {
	...
	if ($post_type == 'statement') {
		...
		if ($subscriptions_payment == "on") {
			...
			$role_plans = wpqa_subscription_role_options();
			echo '<select name="payment_subscribe">
				<option value="-1">'.esc_html__("Subscription plans","wpqa").'</option>';
				if (is_array($role_plans) && !empty($role_plans)) {
					foreach ($role_plans as $key => $value) {
						$key = "wpqa_subscribe-".str_replace("_","-",$key);
						echo '<option '.selected($payment_subscribe,$key,false).' value="'.$key.'">'.$value.' ('.wpqa_meta_count("payment_custom",$key).')</option>';
					}
				}
			echo '</select>';
		}
