...
function wpqa_avatar_name() {
	...
	$avatar = preg_replace('/[^a-z\-\_]/','',strtolower($avatar));
...
if (!function_exists('wpqa_avatar')) :
	function wpqa_avatar($avatar,$id_or_email,$size,$default,$alt) {
		...
		$user_id = (isset($user) && is_object($user) && $user->data->ID > 0?$user->data->ID:0);
		$user_meta_avatar = wpqa_avatar_name();
		if ($user_id > 0) {
			$your_avatar = get_the_author_meta($user_meta_avatar,$user_id);
		}
		if (((isset($your_avatar) && !is_array($your_avatar)) || (isset($your_avatar) && is_array($your_avatar) && isset($your_avatar["id"]) && $your_avatar["id"] > 0)) && $user_id > 0) {
			$avatar = wpqa_get_user_avatar_image($your_avatar,$size,$alt,$user_id);
		}

		if (is_object($id_or_email) && isset($id_or_email->comment_ID)) {
			$anonymously_user = get_comment_meta($id_or_email->comment_ID,"anonymously_user",true);
			if ($anonymously_user != "" || $id_or_email->comment_author_email == "") {
				...
				$default_image = ($default_image_active == "on" && $default_image_anonymous != ""?wpqa_get_aq_resize_url(esc_url($default_image_anonymous),$size,$size):plugin_dir_url(dirname(__FILE__)).'images/default-image.png');
				$avatar = "<img class='avatar avatar-".$size." photo' alt='".esc_attr($alt)."' title='".esc_attr($alt)."' width='".$size."' height='".$size."' src='".$default_image."'>";
			}
		}
		return $avatar;
	}
...
if (!function_exists('wpqa_get_user_avatar_url')) :
	function wpqa_get_user_avatar_url($your_avatar,$size,$user_name) {
		...
		if (isset($your_avatar) && $your_avatar != "" && is_numeric($your_avatar) && $your_avatar > 0) {
			$img_url = wp_get_attachment_image($your_avatar);
			if ($img_url != "") {
				$avatar_num = true;
			}
		}else {
		...
		if (is_numeric($your_avatar) && $your_avatar > 0 && $avatar_num == true) {
			$avatar = wpqa_get_aq_resize_img_url($size,$size,$your_avatar);
		}else {
			if (is_array($your_avatar) && isset($your_avatar['url']) && $your_avatar['url'] != "") {
				$avatar = wpqa_get_aq_resize_url($your_avatar['url'],$size,$size);
			}
		}
		return (isset($avatar) && $avatar != ""?$avatar:"");
	}
endif;
/* Get user avatar image */
if (!function_exists('wpqa_get_user_avatar_image')) :
	function wpqa_get_user_avatar_image($your_avatar,$size,$user_name,$user_id) {
		$array = array(
		...
		return wpqa_get_user_avatar($array);
	}
endif;
...
if (!function_exists('wpqa_get_user_avatar_link')) :
	function wpqa_get_user_avatar_link($args = array()) {
		...
		if (!isset($avatar) || (isset($avatar) && $avatar == "")) {
			...
			if (isset($comment->comment_ID)) {
				...
				if ($anonymously_user != "" || $comment->comment_author_email == "") {
					$user_anonymous = true;
				}
			}
			...
			if ($default_image_active == "on") {
				...
				if (isset($data) && $data == '200') {
					$out_return = true;
				}else if ($default_image_active == "on" && isset($default_image) && $default_image != "") {
					$out_return = false;
				}
			...
			if (isset($user_anonymous) || (!isset($user_anonymous) && $user_id == 0)) {
				$avatar = ($default_image_anonymous != ""?wpqa_get_aq_resize_url(esc_url($default_image_anonymous),$size,$size):plugin_dir_url(dirname(__FILE__)).'images/default-image.png');
			}
		}
