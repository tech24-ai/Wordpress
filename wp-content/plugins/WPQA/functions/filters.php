...
if (!function_exists('wpqa_comment_reply_link')) :
	function wpqa_comment_reply_link($link,$args,$comment) {
		...
		if ($group_id > 0) {
			$link = $args["before"].'<a rel="nofollow" class="comment-reply-link wpqa-reply-link" href="'.wpqa_custom_permalink($comment->comment_post_ID,"view_group_posts","view_group_post").'" data-id="'.$comment->comment_ID.'" data-post_id="'.$comment->comment_post_ID.'" aria-label="'.esc_attr(sprintf($args['reply_to_text'], $comment->comment_author)).'"><i class="icon-reply"></i>'.esc_html__("Reply","wpqa").'</a>'.$args["after"];
		}else {
			$link = $args["before"].'<a rel="nofollow" class="comment-reply-link '.(get_option('comment_registration') && !is_user_logged_in()?'login-panel':'wpqa-reply-link').'" href="'.get_the_permalink($comment->comment_post_ID).'#respond" data-id="'.$comment->comment_ID.'" data-post_id="'.$comment->comment_post_ID.'" aria-label="'.esc_attr(sprintf($args['reply_to_text'], $comment->comment_author)).'"><i class="icon-reply"></i>'.esc_html__("Reply","wpqa").'</a>'.$args["after"];
		}
		return $link;
	}
endif;?>