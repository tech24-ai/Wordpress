...
if (!function_exists('wpqa_categories_checklist')) :
	function wpqa_categories_checklist ($args = array()) {
		...
		$exclude = apply_filters('wpqa_exclude_adding_question_category',$exclude);
			...
			if (isset($question_sort) && is_array($question_sort)) {
				foreach ($question_sort as $sort_key => $sort_value) {
					...
					if ($sort_key == "title_question" && ((isset($question_sort["title_question"]["value"]) && $question_sort["title_question"]["value"] == "title_question") || (isset($question_sort["comment_question"]["value"]) && $question_sort["comment_question"]["value"] != "comment_question"))) {
						...
						$add_poll = wpqa_options("add_poll");
						if (is_user_logged_in()) {
							$user_is_login = get_userdata($user_id);
							$roles = (isset($user_is_login->allcaps)?$user_is_login->allcaps:array());
						}
						if ($is_super_admin || $custom_permission != "on" || ($user_id == 0 && $add_poll == "on") || ($custom_permission == "on" && $user_id > 0 && isset($roles["add_poll"]) && $roles["add_poll"] == 1)) {
							if ($type == "add" && ((!isset($question_add['question_poll']) && isset($_GET['type']) && $_GET['type'] == 'poll') || (isset($question_add['question_poll']) && ($question_add['question_poll'] == "on" || $question_add['question_poll'] == 1)) || (isset($add_question_default["poll"]) && $add_question_default["poll"] == "poll" && empty($question_add)))) {
								$active_poll = true;
		...
		if (is_wp_error($return)) :
   			...
   			if ($post_type == wpqa_questions_type || $post_type == wpqa_asked_questions_type) {
	   			...
	   			if ($get_post->post_status == "draft") {
					...
					wpqa_session('<div class="alert-message alert-message-success"><i class="icon-check"></i><p>'.esc_html__("Your question was successfully added, It's under review.","wpqa").'</p></div>','wpqa_session');
					...
					wpqa_post_after_approved($get_post,(isset($not_user)?$not_user:$post_author));
	...
	if (isset($_POST["form_type"]) && $_POST["form_type"] == "add_question" && isset($_POST["user_id"]) && $_POST["user_id"] != "") :
		...
		if (is_wp_error($return)) :
   			...
   			if ($post_type == wpqa_questions_type || $post_type == wpqa_asked_questions_type) {
   				...
   				if ($get_post->post_status == "draft") {
					...
					wpqa_post_after_approved($get_post);
					$way_sending_notifications = wpqa_options("way_sending_notifications_questions");
					$schedules_time_notification = wpqa_options("schedules_time_notification_questions");
					if ($way_sending_notifications == "cronjob" && $schedules_time_notification != "") {
   	...
   	elseif (isset($_POST["form_type"]) && $_POST["form_type"] == "edit_question") :
		...
		if (is_wp_error($return)) :
			...
			if ($post_status == "draft") {
				wpqa_session('<div class="alert-message alert-message-success"><i class="icon-check"></i><p>'.esc_html__("Your question has been Edited successfully. The question is under review.","wpqa").'</p></div>','wpqa_session');
				wp_redirect(esc_url(home_url('/')));
			}else {
...
function wpqa_process_new_questions($data,$user = "") {
	...
	if ($form_type == "add_question") {
		...
		if (is_user_logged_in()) {
			...
			$user_limit = wpqa_check_user_limit($user_id,$user_is_login,"question");
		...
		if (isset($expired_limit) && $expired_limit == "expired") {
			$errors->add('required','<strong>'.esc_html__("Error","wpqa").':&nbsp;</strong> '.esc_html__("You have reached the maximum for asking questions.","wpqa").' '.wpqa_paid_subscriptions());
		}
		...
		if (!is_user_logged_in() && $ask_question_no_register == "on" && $user_id == 0) {
			...
			if ($pay_ask == "on") {
				...
				do_action("wpqa_update_payment_status",$user_id,$question_id,"question");
			}
...
function wpqa_question($atts, $content = null) {
	...
	$ask_question_no_register = wpqa_options("ask_question_no_register");
	$ask_question = wpqa_options("ask_question");
	$ask_other_users = wpqa_options("ask_other_users");
	$editor_question_details = wpqa_options("editor_question_details");
	$custom_permission = wpqa_options("custom_permission");
	$pay_ask = wpqa_options("pay_ask");
	...
	if ((isset($a["type"]) && $a["type"] == "" && ($is_super_admin || $custom_permission != "on" || ($user_id == 0 && $ask_question == "on") || ($custom_permission == "on" && $user_id > 0 && isset($roles["ask_question"]) && $roles["ask_question"] == 1))) || (isset($a["type"]) && $a["type"] == "user" && ($is_super_admin || $custom_permission != "on" || ($user_id == 0 && $ask_other_users == "on") || ($custom_permission == "on" && $user_id > 0 && isset($roles["ask_other_users"]) && $roles["ask_other_users"] == 1)))) {
		$allow_to_ask_questions = true;
	}
	if (!isset($allow_to_ask_questions)) {
		$out .= wpqa_paid_subscriptions_message(esc_html__("Sorry, you do not have permission to ask a question","wpqa").(!is_user_logged_in()?", ".esc_html__("You must login to ask a question","wpqa"):'').'.');
		if (!is_user_logged_in()) {
			if ($activate_login != "disabled") {
				...
				$out .= '<div class="alert-message alert-message-error"><i class="icon-cancel"></i><p>'.esc_html__("Sorry, you do not have permission to ask a question","wpqa").'.</p></div>';
			}
		}
	}else if (!is_user_logged_in() && (($pay_ask != "on" && $ask_question_no_register != "on") || ($pay_ask == "on" && $unlogged_pay != "on"))) {
		if ($activate_login != "disabled") {
			$out .= wpqa_login_message(esc_html__("You must login to ask a question.","wpqa"),"","",(isset($a["popup"]) && $a["popup"] == "popup" && (has_himer() || has_knowly())?" register='button'":"")).(isset($a["popup"]) && $a["popup"] == "popup"?"":($activate_register != "disabled"?"<div class='pop-footer pop-footer-comments'>".wpqa_signup_in_popup()."</div>":""));
		}else {
			$out .= '<div class="alert-message alert-message-error"><i class="icon-cancel"></i><p>'.esc_html__("Sorry, you do not have permission to ask a question","wpqa").'.</p></div>';
		}
		...
		if (!is_user_logged_in() && $pay_ask == "on" && $unlogged_pay != "on") {
			...
			if (!isset($display_name_ok)) {
				$out .= '<div class="alert-message alert-message-error"><i class="icon-cancel"></i><p>'.esc_html__("You can't ask question to yourself.","wpqa").'</p></div>';
			}else if (!$is_super_admin && isset($_allow_to_ask) && (int)$_allow_to_ask < 1 && $pay_ask == "on" && ($custom_permission != "on" || ($custom_permission == "on" && empty($roles["ask_question_payment"])))) {
				$out .= '<div class="alert-message alert-message-warning"><i class="icon-flag"></i><p>'.esc_html__("Please make a payment to be able to ask a question.","wpqa").'</p></div>';
				...
				if ($ask_payment_style == "packages") {
					...
					$out .= '<a href="'.wpqa_checkout_link("ask_question",(isset($get_user_id) && $get_user_id > 0?$get_user_id:"")).'" class="button-default btn btn__primary" target="'.$payment_pages_target.'">'.esc_html__("Pay to ask a question","wpqa").'</a>';
				}
			}else if (isset($expired_limit) && $expired_limit == "expired") {
				$out .= wpqa_paid_subscriptions_message(esc_html__("You have reached the maximum for asking questions.","wpqa"),"warning");
			}else {
				$question_points_active = wpqa_options("question_points_active");
...
function wpqa_edit_question_attr() {
	...
	if (!is_user_logged_in()) {
		$activate_login = wpqa_options("activate_login");
		if ($activate_login != "disabled") {
			$out .= wpqa_login_message(esc_html__("You must login to edit question.","wpqa"),"warning");
		}else {
			$out .= '<div class="alert-message alert-message-error"><i class="icon-cancel"></i><p>'.esc_html__("Sorry, you do not have permission to edit question.","wpqa").'.</p></div>';
		}
	}else {
			...
			if (isset($get_post) && $get_post != 0 && $get_question && ($post_type == wpqa_questions_type || $post_type == wpqa_asked_questions_type)) {
				...
				if ((($get_question->post_author == $user_id) || ($anonymously_user == $user_id) && $user_id != 0) || $is_super_admin || (isset($moderators_permissions['edit']) && $moderators_permissions['edit'] == "edit")) {
					...
					if ($allow_to_edit_question == true && (((($get_question->post_author == $user_id) || ($anonymously_user == $user_id)) && $user_id != 0 && $get_question->post_status == "publish") || $is_super_admin || (isset($moderators_permissions['edit']) && $moderators_permissions['edit'] == "edit"))) {
						...
						$out .= '<div class="alert-message alert-message-error"><i class="icon-cancel"></i><p>'.esc_html__("Sorry you can't edit this question.","wpqa").'</p></div>';
					}
				}else {
					$out .= '<div class="alert-message alert-message-error"><i class="icon-cancel"></i><p>'.esc_html__("Sorry you can't edit this question.","wpqa").'</p></div>';
				}
			}else {
				$out .= '<div class="alert-message alert-message-error"><i class="icon-cancel"></i><p>'.esc_html__("Sorry no question has been selected or not found.","wpqa").'</p></div>';
			}
		}else {
			$out .= '<div class="alert-message alert-message-error"><i class="icon-cancel"></i><p>'.esc_html__("Sorry, you do not have permission to edit question.","wpqa").'</p></div>';
		}
		...
		if (is_wp_error($return)) :
   			...
   			if ($post_status != "draft" || ($post_status == "draft" && isset($moderators_permissions['edit']) && $moderators_permissions['edit'] == "edit") || is_super_admin($user_id)) {
				...
				if ($post_status == "draft" && isset($moderators_permissions['edit']) && $moderators_permissions['edit'] == "edit") {
					wp_redirect(wpqa_pending_questions_permalink());
				}else {
					wp_redirect(get_permalink($return));
				}
			}else {
...
function wpqa_process_edit_questions($data,$user = "") {
	...
	if ($form_type == "edit_question") {
		...
		wpqa_after_edit_post_approved($get_post_q,($anonymously_user > 0?0:$get_post_q->post_author));
...
function wpqa_add_edit_question_errors($errors,$posted,$type,$question_sort,$get_question_user_id,$comment_question,$title_question) {
	...
	if ($type == "add") {
		if (isset($question_sort["video_desc_active"]["value"]) && $question_sort["video_desc_active"]["value"] == "video_desc_active" && isset($posted['video_description']) && $posted['video_description'] == "on" && empty($posted['video_id'])) {
		...
		if (isset($question_sort["terms_active"]["value"]) && $question_sort["terms_active"]["value"] == "terms_active" && isset($posted['terms_active']) && $posted['terms_active'] != "on") {
