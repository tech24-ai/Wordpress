<?php if ($under_construction == "on") {
	...
	$form_style         = discy_post_meta("form_style");
	...
	$form_style         = discy_options("form_style");
...
<div class="the-main-container">
	...
	<main class="discy-login-wrap<?php echo (isset($_POST["form_type"]) && $_POST["form_type"] == "wpqa-signup"?" page-signup-wrap":"").($confirm_email == "yes" || $register_style == "style_2"?" discy-login-2":"").($under_construction == "on"?" under-construction":"")?>">
		...
		if ($under_construction != "on" && is_super_admin()) {
			echo '<div class="alert-message alert-message-warning"><i class="icon-flag"></i><p>'.esc_html__("This page must be redirected to home if you are logged in, but it's not redirecting for the admin.","discy").'</p></div>';
		}
		if ($under_construction != "on" && $confirm_email == "yes") {
			...
			<div class="<?php echo ($register_style == "style_2"?"":"centered")?>">
				...
				<?php if ($under_construction != "on" && !is_user_logged_in()) {?>
