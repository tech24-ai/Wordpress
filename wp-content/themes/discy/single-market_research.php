				...
				while ( have_posts() ) : the_post();
					...
					*/ ?>
					...
					<div class="col col5" id="mr-meta"> 
						...
						<?php $research_categories = get_market_research_post_categories($post->ID);
						
							if (count($research_categories) > 1)
							{
								echo '<div class="reserach-categories-data">';
									foreach ($research_categories as $index => $category)
									{
										echo '<h5><a href="/market-research-category/'. $category['slug'] .'" style="text-transform: uppercase;">'.$category['name'].'</a></h5>';
										if ($index < count($research_categories) - 1)
										{
											echo '&nbsp, ';
										}
									}
							...
							else if(count($research_categories)==1)
							{
								echo '<div class="reserach-categories-data">';
									echo '<h5><a href="/market-research-category/'. $research_categories[0]['slug'] .'" style="text-transform: uppercase;">'.$research_categories[0]['name'].'</a></h5>';
								echo '</div>';
							}
					...
					<div class="col col8" id="mr-content"> 
						<?php echo get_the_content(); 
							$post_market_research_tags = get_market_research_tags($post->ID);
							if ( ! is_wp_error( $post_market_research_tags ) && ! empty( $post_market_research_tags ) ) {
								echo '<hr>';
								echo '</br>';
								echo '<span>TAGS: </span>';
								foreach ( $post_market_research_tags as $tag ) {
									echo '<a class="mrkt-tag" href="' . esc_url( get_term_link( $tag->term_id ) ) . '">' . esc_html( $tag->name ) . ' </a> ';
								}
							}
						?>
					</div>
					...
					<div class="col col4" id="recom-title"> 
						...
						<?php if(count(fetch_similar_related_market_researches($post->ID))):?>
						<div class="recommended-data-related-research-cat">
							<?php foreach(fetch_similar_related_market_researches($post->ID) as $i=>$market_research):?>
								<u><b><a href="<?=$market_research['slug']?>"><?=$market_research['name']?></a></b></u><br>
							<?php endforeach;?>
						</div>
						<?php endif;?>
					</div>
